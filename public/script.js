 ...
 ***********************/
const LS_KEYS = {
    PROGRESS: "compassProgress",      // odpovědi pole [-3..3]
...
function showQuestion() {
    if (testState.current < testState.order.length) {
        ...
        const progressText = currentLang === "cs"
        ...
        byId("progress").setAttribute("aria-label", progressText);
...
function renderResults() {
    ...
    if (scores && Object.keys(scores).length) {
        ...
        AXIS_ORDER.forEach((axis) => {
            ...
            // Každá osa má 16 otázek a odpověď má rozsah -3 až +3.
            const percent = Math.max(-100, Math.min(100, Math.round((val / 48) * 100)));
            const axisDef = AXIS_LABELS[axis];

            const wrapper = document.createElement("div");
            wrapper.className = "axis-result";

            const label = document.createElement("div");
            label.className = "axis-label";
            label.innerHTML = `
                <span class="axis-side">${axisDef.left[lang]}</span>
                <span class="axis-score">${Math.abs(percent)}%</span>
                <span class="axis-side">${axisDef.right[lang]}</span>
            `;
            ...
            bar.setAttribute(
                "aria-label",
                `${axisDef.left[lang]} – ${axisDef.right[lang]}: ${Math.abs(percent)}% ${percent < 0 ? axisDef.left[lang] : percent > 0 ? axisDef.right[lang] : (lang === "cs" ? "střed" : "center")}`
            );
            ...
            fill.className = "fill";
            fill.classList.add(percent < 0 ? "is-left" : "is-right");
            ...
            if (axisDef.desc) {
                ...
                text.className = "axis-description";
                text.innerHTML = `
                    <div><b>${axisDef.left[lang]}:</b><br>${axisDef.desc[lang].left}</div>
                    <div><b>${axisDef.right[lang]}:</b><br>${axisDef.desc[lang].right}</div>
                `;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (clearBtn) {
        clearBtn.addEventListener("click", () => {
            resultsClearedManually = true;
            localStorage.removeItem(LS_KEYS.RESULTS);
            localStorage.removeItem(LS_KEYS.PROGRESS);
            localStorage.removeItem(LS_KEYS.ORDER);

            document.getElementById("bars").innerHTML = `
                <div class="notice" data-i18n-html="results_cleared">
                    Výsledky byly smazány. <a href="test.html">Znovu vyplnit test</a>.
                </div>
            `;
